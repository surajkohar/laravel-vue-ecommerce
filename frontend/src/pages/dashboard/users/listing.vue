  ...
  <div class="products-listing">
    <div class="header-section">
      ...
      <div class="action-buttons">
        <button class="add-button" @click="router.push('/admin/user/add')">
    ...
    <div class="d-flex justify-content-between align-items-center mb-2">
      ...
      <div class="search-container">
        <input type="text" v-model="filters.search" @input="fetchUsers" placeholder="Search users..." class="search-input">
      </div>
    </div>
      ...
      <table>
        ...
        <tr>
          ...
          <th @click="sortBy('name')">
            Email
            <span class="sort-icon">
              <template v-if="filters.sort_field === 'email'">
                {{ filters.sort_direction === 'asc' ? '↑' : '↓' }}
              </template>
              <template v-else>↑↓</template>
            </span>
        ...
        <tbody>
          <tr v-for="role in users.data" :key="role.id">
            ...
            <td class="actions-cell">
              ...
              <button class="edit-button" @click="editUser(role.id)">
                <svg xmlns="http://www.w3.org/2000/svg" width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round">
    ...
    <!-- Pagination -->
    <div class="pagination" v-if="users.meta && users.meta.last_page > 1">
      <button @click="prevPage" :disabled="users.meta.current_page === 1">Previous</button>
      <span v-for="page in visiblePages" :key="page" 
            @click="goToPage(page)" 
            :class="{ active: users.meta.current_page === page }">
        {{ page }}
      </span>
      <button @click="nextPage" :disabled="users.meta.current_page === users.meta.last_page">Next</button>
    </div>

    <!-- Empty State -->
    <div v-if="!loading && (!users.data || users.data.length === 0)" class="empty-state">
...
const fetchUsers = async () => {
  try {
    ...
    const response = await fetch(`${API.BACKEND_URL}/users?${queryParams.toString()}`, {
      method: 'GET',
      headers: {
...
// Pagination methods
const prevPage = () => {
  if (users.value.meta.current_page > 1) {
    filters.value.page = users.value.meta.current_page - 1;
    fetchUsers();
  }
};

const nextPage = () => {
  if (users.value.meta.current_page < users.value.meta.last_page) {
    filters.value.page = users.value.meta.current_page + 1;
    fetchUsers();
  }
};
...
const goToPage = (page) => {
  if (page !== users.value.meta.current_page) {
