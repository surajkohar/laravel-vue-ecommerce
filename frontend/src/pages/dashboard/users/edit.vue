              ...
              <div class="form-group">
                <div class="password-options">
                  <label class="radio-option">
                    <input type="radio" v-model="passwordOption" value="keep" />
                    <span class="radio-custom"></span>
                    <span class="radio-label">Keep current password</span>
                  </label>
                  <label class="radio-option">
                    <input type="radio" v-model="passwordOption" value="change" />
                    <span class="radio-custom"></span>
                    <span class="radio-label">Change password</span>
                  </label>
                </div>
              </div>

              <div class="form-group" v-if="passwordOption === 'change'">
          ...
          <div class="card profile-card">
            <div class="card-body">
              ...
              <div class="profile-upload-wrapper">
                <div class="avatar-upload">
                  <div
                    ...
                    "
                  >
                    <div v-if="!previewImage && !user.profile_image" class="initials">
                      {{ userInitials }}
                    </div>
                  </div>
                  <div class="avatar-upload-controls">
                    <label for="avatarUpload" class="upload-btn">
                      <i class="fas fa-camera"></i> {{ user.profile_image ? 'Change Photo' : 'Upload Photo' }}
                    </label>
                    <input
                      id="avatarUpload"
                      type="file"
              ...
              <div class="additional-options">
                ...
                <div class="form-group">
                  <label>Send Password Update Email</label>
                  <label class="switch">
                    <input type="checkbox" v-model="sendUpdateEmail" />
                    <span class="slider round"></span>
                  </label>
                  <span class="toggle-label">{{
                    sendUpdateEmail ? "Yes" : "No"
                  }}</span>
                </div>

                <div class="form-group" v-if="sendUpdateEmail">
                  <label>Custom Message</label>
                  <textarea
                    v-model="updateMessage"
                    rows="3"
                    placeholder="Optional custom message for the update email"
                  ></textarea>
                </div>
              </div>
            </div>
...
const sendUpdateEmail = ref(false);
...
const prepareFormData = () => {
  ...
  if (passwordOption.value === 'change' && user.value.password) {
  ...
  } else if (previewImage.value === null && user.value.profile_image === null) {
    // This indicates we want to remove the existing image
    formData.append("remove_profile_image", "1");
  }

  // Add update email options
  formData.append("send_update_email", sendUpdateEmail.value);
  if (sendUpdateEmail.value && updateMessage.value) {
    formData.append("update_message", updateMessage.value);
  }
...
const saveUser = async () => {
  ...
  try {
    ...
    const response = await fetch(`${API.BACKEND_URL}/user/${route.params.id}/update`, {
    ...
    if (response.ok) {
      ...
      if (data.status) {
        user.value = {
          ...
          profile_image: data.data.profile_image // This now contains the full URL
        };
      }
    } else {
