              ...
              <div class="form-group">
                ...
                <div class="image-uploader">
                  <div class="upload-area" @click="triggerMainImageUpload"
                    ...
                    <template v-else>
                      <img :src="product.mainImageFile ? product.mainImageFile.preview : product.mainImageUrl"
                        alt="Main product image preview" class="main-image-preview" />
                    </template>
                    <input type="file" ref="mainImageInput" @change="handleMainImageUpload" accept="image/*"
                      style="display: none" />
                  </div>
                  <button v-if="product.mainImageUrl || product.mainImageFile" class="btn btn-danger btn-sm mt-2"
              ...
              <div class="form-group">
                ...
                <div class="file-uploader">
                  <div class="upload-area" @click="triggerSizeGuideUpload"
                    ...
                    <template v-else>
                      <i class="fas fa-file-pdf fa-3x"></i>
                      <p class="file-name">{{ product.sizeGuideFile ? product.sizeGuideFile.name : product.sizeGuideName
                        }}</p>
                    </template>
                    <input type="file" ref="sizeGuideInput" @change="handleSizeGuideUpload" accept=".pdf"
                      style="display: none" />
                  </div>
                  <button v-if="product.sizeGuideUrl || product.sizeGuideFile" class="btn btn-danger btn-sm mt-2"
          ...
          <div class="color-management">
            ...
            <div class="variant-details" v-if="activeVariant">
              ...
              <div class="size-selection mb-4">
                <h5 class="mb-3">Available Sizes for {{ getActiveVariant().color_name || getColorName(activeVariant) }}
                </h5>
                    ...
                    <tbody>
                      <tr v-for="size in sizes" :key="size.id">
                        ...
                        <td>
                          <div class="input-group" v-if="isSizeSelected(activeVariant, size)">
                            <input type="number" class="form-control"
                              v-model.number="getSizeObject(activeVariant, size).price" placeholder="0.00" min="0"
                              step="0.01" />
                          </div>
                        ...
                        <td>
                          <div class="input-group" v-if="isSizeSelected(activeVariant, size)">
                            <input type="number" class="form-control"
                              v-model.number="getSizeObject(activeVariant, size).stock" placeholder="0" min="0" />
                          </div>
                          <span v-else class="text-muted">-</span>
                        </td>
                        <td>
                ...
                <div class="image-uploader">
                  ...
                  <div class="image-previews mt-3">
                    ...
                    <div v-for="(image, index) in getActiveVariant().newImages" :key="'new-' + index"
                      ...
                      <button class="btn btn-danger btn-sm remove-image" @click="removeNewImage(activeVariant, index)">
                        <i class="fas fa-times"></i>
                      </button>
                    </div>
                  </div>
